string folderPath = @"C:\vs22data\Week8EpicGame";
string heroFile = "heroes.txt";
string villainFile = "villains.txt";

string[] heroes = File.ReadAllLines(Path.Combine(folderPath, heroFile));
string[] villains = File.ReadAllLines(Path.Combine(folderPath, villainFile));
string[] weapon = { "m1911", "m16", "Ixtal Spear", "Laser", "M60" };
...
Console.WriteLine($"Today {villain} ({villainHP}) with {villainWeapon} tries to end the world!");
...
{
    Console.WriteLine($"World go brrrr (villain wins)");
}
else
{
    Console.WriteLine("Draw! (everyone loses)");
}
